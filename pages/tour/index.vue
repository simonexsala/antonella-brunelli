<script setup>
const { data: scritti } = await useAsyncData('tour', () =>
  queryContent('/tour').sort({ date: -1 }).find()
)

const title = ref('Tour / Antonella Brunelli')
const description = ref('Tour')
useHead({
  title,
  meta: [
    {
...
<template>
  <div class="container flex-1 max-w-3xl mt-2 px-6 mx-auto space-y-12 xl:max-w-5xl">
    <main>
      <div>
        <h2 class="underline decoration-2 decoration-violet-300 text-3xl font-extrabold text-center">Tour</h2>
      </div>
